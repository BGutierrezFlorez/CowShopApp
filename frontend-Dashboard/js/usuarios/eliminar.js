...
import { usuariosGlobal, cargarUsuarios } from './cargar.js';
...
export function configurarEliminar(getPageFn) {
  ...
  if (!tablaBody) {
    console.error('CRÍTICO: No se encontró el tbody #tabla-usuarios para configurar los eventos.');
    return;
  }
  ...
  tablaBody.addEventListener('click', async e => {
    ...
    try {
      ...
      if (resp.status === 204) {
        ...
        mostrarPagina(paginaActual, usuariosGlobal, pageSize, renderTabla, renderPaginacion);
