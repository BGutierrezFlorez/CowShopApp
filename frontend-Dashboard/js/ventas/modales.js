...
import { actualizarVenta } from './editar.js';

export function abrirModalEditar(venta) {
      ...
      <form class="modal-content" id="formEditarVenta">
        ...
        <div class="modal-body">
          <div class="mb-3">
            ...
            <input type="number" class="form-control" name="idComprador" value="${venta.ID_Comprador}" required>
          </div>
          <div class="mb-3">
            <label class="form-label">Fecha Venta</label>
            <input type="date" class="form-control" name="fechaVenta" value="${(venta.Fecha_Venta||'').slice(0,10)}" required>
          </div>
          <div class="mb-3">
  ...
  form.addEventListener('submit', async e => {
    ...
    try {
      await actualizarVenta(venta.ID_Venta, datos);
